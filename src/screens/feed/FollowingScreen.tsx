...
type RouteT = RouteProp<FeedStackParamList, 'Following'>;
...
export default function FollowingScreen() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const { data } = await apiClient.get(API_ENDPOINTS.USER_FOLLOWING(userId), { params: { page: 0, size: 50 } });
  ...
  return (
    <SafeAreaView style={styles.safe}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>{userName ? `${userName} is` : ''} Following</Text>
        <View style={{ width: 34 }} />
      </View>
