...
**Open bugs:** None — BUG-MOB-021 fixed this session (Jul 20, 2026), but note: the extension half of the fix (`applyai-extension` `src/content/webapp.ts`) won't reach real users until a new version is built and republished to the Chrome Web Store (extension has been live there since Jul 1, 2026 — see applyai-extension BUILD_LOG.md).
**Last push:** Jul 12, 2026 — `57cd1ee` (BUG-MOB-014/015, Home tab collision + job feed dedup), on `main`. `feature/web-app` branch (FEAT-023/024/025 + this session's BUG-MOB-018/019/020) committed locally, not yet pushed — held pending user confirmation since it's a large WIP branch. This session's BUG-MOB-021 fix (Jul 20, 2026) is also uncommitted, pending user confirmation.
