...
| BUG-MOB-025 (cross-repo, backend BUG-075) | Live-verifying the backend's BUG-074 fix (bulk auto-apply queue timeout), a real 19-job batch against the deployed web app produced 9 FAILED items with zero visible reason in the UI — the extension's autopilot marked them FAILED, but nothing on screen said why. Backend's BUG-073 (Jul 22) had persisted a real `errorMessage` to the DB, but `AutoApplyQueueResponse` never exposed it — confirmed live by fetching the raw API JSON in the browser console and seeing the field simply absent | AutoApplyQueueScreen.tsx, types/api.types.ts | Aug 10, 2026 | ✅ FIXED Aug 10, 2026 — `AutoApplyQueueItem.errorMessage` added; `QueueCard` renders it as a red reason line under FAILED/SKIPPED items (`testID="error-reason-{id}"`) once the backend fix (commit c7e1acf) exposes the field. 2 new AutoApplyQueueScreen tests (renders reason text; renders nothing when null). 564 mobile tests green |
...
| BUG-MOB-021 | `AutoApplyQueueScreen`'s "Install the ApplyAI Chrome extension" banner rendered unconditionally on web — there was never any detection logic, so it kept showing even when the user already had the extension installed. Found from a user screenshot (20 queued jobs, banner still showing) | AutoApplyQueueScreen | Jul 20, 2026 | ✅ FIXED Jul 20, 2026 — new `useExtensionInstalled` hook (web-only) pings the extension via `window.postMessage`; the extension's new `src/content/webapp.ts` content script (applyai-extension repo) replies from the ApplyAI web app's own origin. Banner now hides once the reply arrives; native keeps showing it unconditionally (no browser/extension concept there). 523 mobile tests green (519 + 4 new: useExtensionInstalled.test.ts ×3, AutoApplyQueueScreen banner-hidden case). Requires a new Chrome Web Store release of the extension to reach already-installed users — see CURRENT STATUS. |
| BUG-MOB-022 | Found live testing web sign-in via Claude in Chrome: clicking "Continue with Google" left the button spinning forever (once, the tab froze entirely). Console: `FedCM get() rejects with AbortError: signal is aborted without reason`, no notification ever delivered | GoogleSignInScreen (utils/googleWebAuth.ts) | Jul 20, 2026 | ✅ FIXED Jul 20, 2026 — `signInWithGoogleWeb()` had no timeout at all; wrapped the whole flow (script load included) in an 8s timeout that always settles with an actionable error. Opted into `use_fedcm_for_prompt: true`; distinguished real user dismissal (silent "Sign-in cancelled") from Google never showing anything (now a visible error). New `googleWebAuth.test.ts` (5 tests). 528 mobile tests green (523 + 5 new) |
| BUG-MOB-023 | User asked to "store the tailored resume for interview preparation" — paired with applyai-backend BUG-071 (tailored resume text was never persisted to the Application record, only ever lived on the auto-apply queue row) | ApplicationDetailScreen | Jul 20, 2026 | ✅ FIXED Jul 20, 2026 — added `tailoredResumeText` to the `Application` type and a new collapsible "Tailored Resume" section in `ApplicationDetailScreen.tsx`, mirroring the existing Cover Letter section. 530 mobile tests green (528 + 2 new) |
| FEAT-026 | User feedback with screenshots after the above session: "most of pages are still not looking good in web application" — turned out FEAT-024/025's `WebPageContainer` centering treatment had only ever reached 7 of 44 screens (Home, Feed, JobFeed, ResumeList, ApplicationsList, InterviewStart, GoogleSignIn); every other screen (JobDetail, ProfileSettings, ResumeDetail, ApplicationDetail, all the AI-tool screens, all of Feed's sub-screens, etc.) was still stretching full-bleed edge-to-edge on a wide browser window | 36 screens across every folder in src/screens | Jul 13, 2026 | ✅ Complete Jul 13, 2026 — applied `WebPageContainer` to all 36 remaining screens. `WebPageContainer` gained an optional `style` prop (a `gap` set on a ScrollView's `contentContainerStyle` stops applying once content sits one level deeper inside the wrapper — pass `style={{gap: N}}` on the container itself instead). Deliberately skipped: `OnboardingScreen` (horizontal `pagingEnabled` carousel — wrapping risks breaking the paging width math) and `SplashScreen` (already a centered logo animation, nothing to constrain). Verified live in the user's real Chrome: SalaryIntel, Blacklist, HrPostJob, Profile all confirmed now render as a properly centered column. 517 mobile tests green (unchanged — layout-only, no new test surface) |
| BUG-MOB-024 | Production 500 on resume upload (`applyai-mobile.vercel.app/resumes/upload`) — user screenshot showed "An internal error occurred", network panel showed the upload request returning 500. Railway backend logs showed the real exception: `MissingServletRequestPartException: Required part 'file' is not present`. Root cause: `ResumeUploadScreen.upload()` always built the React Native native-platform multipart shape (`formData.append('file', {uri, name, type})`) — on native, RN's `FormData` polyfill turns that descriptor into a real multipart file part, but on web `FormData` is the browser's real implementation, which just stringifies a plain object into a text field named "file" instead of a file part, so Spring never saw an actual file | ResumeUploadScreen | Aug 9, 2026 | ✅ FIXED Aug 9, 2026 — on `Platform.OS === 'web'`, now appends the real `File` object `expo-document-picker` already provides on web (`asset.file`, confirmed present in `expo-document-picker`'s type defs) via `formData.append('file', selectedFile.file, selectedFile.name)`; native path unchanged. Shows an explicit error instead of silently failing if `asset.file` is ever missing. New regression tests in ResumeUploadScreen.test.tsx (web File appended correctly; missing-file error path) — 564 mobile tests green (562 + 2 new) |
...
**Open bugs:** None — BUG-MOB-025 (FAILED auto-apply items had no visible reason, cross-repo with backend BUG-075) fixed Aug 10, 2026, found while live-verifying the backend's BUG-074 fix on the deployed site.
**Last push:** Aug 10, 2026 — BUG-MOB-025 fix (AutoApplyQueueScreen renders errorMessage), on `main`.
...
## SESSION — Jul 20, 2026 — BUG-MOB-021: extension install-detection (cross-repo w/ applyai-extension)

**Trigger:** User screenshot — AutoApplyQueueScreen showing "Install the ApplyAI Chrome extension" with 20 jobs queued, extension already installed. `AutoApplyQueueScreen.tsx`'s hint banner had no detection logic at all; it rendered unconditionally whenever `!selectMode`.

**Fix:** New `hooks/useExtensionInstalled.ts` — web-only (native always returns not-installed, since there's no browser/extension concept there and the hint stays meaningful telling native users to switch to desktop Chrome). Pings `window.postMessage({ type: 'APPLYAI_EXTENSION_PING' })` and waits 400ms for `APPLYAI_EXTENSION_PONG`. The reply comes from applyai-extension's new `src/content/webapp.ts` content script (shipped in that repo's v1.0.3, same session) — a content script matched only to the ApplyAI web app's own origin. Banner in `AutoApplyQueueScreen.tsx` now conditioned on `!extensionInstalled`.

**Tests:** `useExtensionInstalled.test.ts` (3 new: native skips ping, web detects install via reply, web times out to not-installed) + one new `AutoApplyQueueScreen.test.tsx` case (banner hidden when hook reports installed). 523 mobile tests green (519 + 4 new).

**Cross-repo caveat:** This is a code fix only. Real users won't see the banner disappear until applyai-extension's v1.0.3 is submitted to the Chrome Web Store (still serving v1.0.2) — a manual step for the user via the Developer Dashboard, not something Claude can do.
