...
**Next to build:** FEAT-023 Phase 1 completion (Google Cloud Console + Railway CORS setup, then verify web login end-to-end) → Phase 2 (core job-search + auto-apply loop on web, the actual validation milestone for the whole web-app pivot). Google Play Store submission still pending separately (register account ₹2,100 → EAS production build).
**Blocked on:** `actions/ACTION_REQUIRED_004.md` — Google Cloud Console (Authorized JavaScript origins) + Railway `CORS_ALLOWED_ORIGINS`, both external actions only the user can do. Everything past the login screen on web is unverifiable until this is done.
**Open bugs:** None
**Last push:** Jul 12, 2026 — `57cd1ee` (BUG-MOB-014/015, Home tab collision + job feed dedup), on `main`. FEAT-023 (web app bootstrap, Phase 0+1) is on a new branch `feature/web-app`, committed locally, not yet pushed — held pending user confirmation since it's a large WIP branch.
