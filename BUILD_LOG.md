...
**Next to build:** Continue the web-parity screen sweep — verified so far (this session, live in the user's real Chrome): Onboarding, GoogleSignIn/login, Home, Feed, Jobs list + JobDetail, Resumes list, Applications, Interview (Mock Interview landing), Profile, ProfileSettings. ~25 screens still unverified on web: CoverLetter, TailorResume, ResumeUpload, ResumeDetail, InterviewQuestion (voice recorder — web mic permission untested), InterviewReport, CompanyIntel, SalaryIntel, NegotiationCoach, CareerPath, Analytics, Blacklist, JobAlerts, SavedJobs, MatchScore, HR screens (HrMyJobs/HrPostJob), AutoApplyQueue, Notifications, Paywall (partially — restore-purchases path fixed, purchase flow itself is native-only by design), chat/social screens (Feed sub-screens, PostDetail, ChatDetail). Also noted but not yet fixed: ResumeListScreen's delete-resume is `onLongPress`-only with no visible icon, unintuitive/undiscoverable for a desktop mouse user (no dedicated bug ID yet — flagging for a follow-up session). Google Play Store submission still pending separately (register account ₹2,100 → EAS production build).
